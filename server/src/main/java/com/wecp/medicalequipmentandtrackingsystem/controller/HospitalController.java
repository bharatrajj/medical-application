...
public class HospitalController {
    ...
    public ResponseEntity<OrderData> placeOrder(@RequestParam Long equipmentId, @RequestBody OrderData order) {
        // place order for the equipment and return the placed order with status code 201 = CREATED;
        try {
            return new ResponseEntity<OrderData>(orderService.placeOrder(equipmentId, order),HttpStatus.CREATED);
        } catch (SQLException e) {
         return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
