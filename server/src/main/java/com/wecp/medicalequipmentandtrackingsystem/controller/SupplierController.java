...
public class SupplierController {
    ...
    public ResponseEntity<List<OrderData>> getAllOrders() {
        List<OrderData> a=orderService.getAllOrders();
        return new ResponseEntity<>(a,HttpStatus.OK);
        // get all order and return it status code 200 OK
    }
    @PutMapping("/api/supplier/order/update/{orderId}")
    public ResponseEntity<OrderData> updateOrderStatus(@PathVariable Long orderId, @RequestParam String newStatus) {
        OrderData b=orderService.updateOrderStatus(orderId, newStatus);
